...
export function buildMonthlyDataSnapshot(uploadsDir) {
  const candidates = [];
  const csvNames = CSV_CANDIDATES;
  const dir = uploadsDir || resolveRepoPath('uploads');
  ...
  try {
    if (fileExists(dir)) {
      for (const entry of fs.readdirSync(dir)) {
        if (entry.toLowerCase().endsWith('.csv')) candidates.push(path.join(dir, entry));
      }
    }
  } catch {}
  ...
  const chosen = chooseLatestFile(candidates);
  if (!chosen) {
    return { found: false, filepath: null, reportDate: null, products: [], discoveredProductNames: [] };
  }
  const buffer = fs.readFileSync(chosen);
  const parsed = parsePensumCsvBuffer(buffer);
  return { found: true, filepath: chosen, ...parsed };
}
