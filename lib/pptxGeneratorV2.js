...
const PRODUCT_COLORS = ['1B3A5F', '5B9BD5', 'D4886B', '0D9488', 'B8860B', '7C3AED', '2E7D32'];
...
function addHeader(pptx, slide, title, subtitle = '') {
  ...
  slide.addText(title, { x: 0.7, y: 0.35, w: 11.5, h: 0.45, fontFace: 'Aptos', fontSize: 22, bold: true, color: COLORS.navy });
  if (subtitle) slide.addText(subtitle, { x: 0.7, y: 0.82, w: 11.5, h: 0.22, fontFace: 'Aptos', fontSize: 10, color: COLORS.muted });
...
function addFooter(pptx, slide, pageNum = '') {
  ...
  slide.addText('Illustrativ modellportefølje – ikke personlig investeringsråd eller egnethetsvurdering.', {
    x: 0.7, y: 6.82, w: 9, h: 0.18, fontFace: 'Aptos', fontSize: 7, color: COLORS.muted,
  });
  slide.addText('Pensum Asset Management AS', {
    x: 10.2, y: 6.82, w: 2.8, h: 0.18, fontFace: 'Aptos', fontSize: 7, color: COLORS.muted, align: 'right',
  });
  if (pageNum) {
    slide.addText(pageNum, { x: 12.5, y: 6.82, w: 0.5, h: 0.18, fontFace: 'Aptos', fontSize: 7, color: COLORS.muted, align: 'right' });
  }
}
...
function addBullets(slide, items, x, y, w, opts = {}) {
  ...
  slide.addText(runs, {
    x, y, w, h: opts.h || 2.5,
    fontFace: 'Aptos', fontSize: opts.fontSize || 11, color: COLORS.text,
...
function addKpiCards(pptx, slide, metrics = [], y = 1.5, opts = {}) {
  ...
  metrics.slice(0, count).forEach((metric, idx) => {
    ...
    slide.addText(metric.label || 'KPI', {
      ...
      fontFace: 'Aptos', fontSize: 9, color: COLORS.muted,
    });
    // Value
    slide.addText(metric.value || '–', {
      x: x + 0.15, y: y + 0.4, w: width - 0.3, h: 0.35,
      fontFace: 'Aptos', fontSize: 20, bold: true, color: COLORS.navy,
    });
  });
}
  ...
  if (opts.showLegend !== false) {
    ...
    data.forEach((d, i) => {
      ...
      slide.addText(d.name, {
        ...
        fontFace: 'Aptos', fontSize: 9, color: COLORS.text,
      });
      slide.addText(`${d.value.toFixed(1)}%`, {
        x: legendX + (opts.legendW || 2.2) + 0.1, y: ly, w: 0.7, h: 0.22,
        fontFace: 'Aptos', fontSize: 9, bold: true, color: COLORS.navy, align: 'right',
      });
    });
  }
}
...
function addHorizontalBar(pptx, slide, data, x, y, w, opts = {}) {
  ...
  data.forEach((d, i) => {
    ...
    slide.addText(d.name, {
      ...
      fontFace: 'Aptos', fontSize: 9, color: COLORS.text, align: 'left', valign: 'mid',
    });
    // Background bar
    slide.addShape(pptx.ShapeType.roundRect, {
    ...
    slide.addText(`${d.value.toFixed(1)}%`, {
      ...
      fontFace: 'Aptos', fontSize: 8, bold: true, color: COLORS.navy, align: 'right', valign: 'mid',
    });
  });
}
...
export async function generateProposalPptxV2(model) {
  ...
  slide.addText('Illustrativ\ninvesteringsskisse', {
    ...
    fontFace: 'Aptos', fontSize: 36, bold: true, color: COLORS.navy,
  ...
  slide.addText(`Pensum Asset Management AS  |  ${model?.meta?.reportDate || ''}`, {
    ...
    fontFace: 'Aptos', fontSize: 10, color: COLORS.muted,
  });
  ...
  infoCards.forEach((card, i) => {
    ...
    slide.addText(card.label, { x: 9.4, y: cy + 0.12, w: 3.1, h: 0.18, fontFace: 'Aptos', fontSize: 9, color: COLORS.muted });
    slide.addText(card.value, { x: 9.4, y: cy + 0.38, w: 3.1, h: 0.32, fontFace: 'Aptos', fontSize: 18, bold: true, color: COLORS.navy });
  });
  ...
  const tableRows = (model.productNodes || []).map((p, i) => [
    { text: p.name, options: { fontFace: 'Aptos', fontSize: 10, color: COLORS.text, bold: true } },
    { text: `${(p.weight * 100).toFixed(1)}%`, options: { fontFace: 'Aptos', fontSize: 10, color: COLORS.navy, bold: true, align: 'center' } },
    { text: p.roleInPortfolio || '–', options: { fontFace: 'Aptos', fontSize: 9, color: COLORS.muted } },
    { text: p.benchmark || '–', options: { fontFace: 'Aptos', fontSize: 9, color: COLORS.muted } },
  ]);
  const headerRow = [
    { text: 'Produkt', options: { fontFace: 'Aptos', fontSize: 9, bold: true, color: COLORS.white, fill: { color: COLORS.navy } } },
    { text: 'Vekt', options: { fontFace: 'Aptos', fontSize: 9, bold: true, color: COLORS.white, fill: { color: COLORS.navy }, align: 'center' } },
    { text: 'Rolle', options: { fontFace: 'Aptos', fontSize: 9, bold: true, color: COLORS.white, fill: { color: COLORS.navy } } },
    { text: 'Benchmark', options: { fontFace: 'Aptos', fontSize: 9, bold: true, color: COLORS.white, fill: { color: COLORS.navy } } },
  ];
  ...
  for (const product of model.productNodes || []) {
    ...
    slide.addText('Nøkkelinformasjon', {
      ...
      fontFace: 'Aptos', fontSize: 11, bold: true, color: COLORS.navy,
    });
    const infoItems = [
    ...
    infoItems.forEach(([label, val], i) => {
      ...
      slide.addText(label, { x: 8.7, y: iy, w: 1.5, h: 0.2, fontFace: 'Aptos', fontSize: 9, color: COLORS.muted });
      slide.addText(val, { x: 8.7, y: iy + 0.2, w: 3.8, h: 0.22, fontFace: 'Aptos', fontSize: 10, bold: true, color: COLORS.navy });
    });
    ...
    if ((product.blueprint?.slideCount || 0) > 1) {
      ...
      positions.forEach((pos) => {
        ...
        slide.addText(pos.label.charAt(0).toUpperCase() + pos.label.slice(1), {
          ...
          fontFace: 'Aptos', fontSize: 10, bold: true, color: COLORS.navy,
        });
        // Placeholder text
        slide.addText('Data kobles fra Morningstar / intern kilde', {
          x: pos.x + 0.15, y: pos.y + pos.h / 2 - 0.1, w: pos.w - 0.3, h: 0.25,
          fontFace: 'Aptos', fontSize: 9, color: COLORS.muted, align: 'center',
        });
      });
  ...
  steps.forEach((step, i) => {
    ...
    slide.addText(String(i + 1), {
      x: 0.9, y: sy, w: 0.4, h: 0.4,
      fontFace: 'Aptos', fontSize: 14, bold: true, color: COLORS.white, align: 'center', valign: 'mid',
    });
    // Step text
    slide.addText(step, {
      x: 1.55, y: sy + 0.03, w: 10.5, h: 0.4,
      fontFace: 'Aptos', fontSize: 12, color: COLORS.text,
    });
  });
