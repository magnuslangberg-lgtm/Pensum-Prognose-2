...
export function addDonutChart(pptx, slide, data, x, y, opts = {}) {
  ...
  const total = donutPairs.reduce((sum, d) => sum + d.value, 0) || 1;
  const cx = 240;
  const cy = 240;
  const radius = 170;
  const innerRadius = radius * ((opts.holeSize || 55) / 100);
  let angle = -Math.PI / 2;
  const svgSlices = donutPairs.map((d, i) => {
    ...
    const color = safeColor(filtered[i].color || PRODUCT_COLORS[i % PRODUCT_COLORS.length]);
