...
function buildOverrideMap(pensumProdukter = []) {
  const arr = Array.isArray(pensumProdukter) ? pensumProdukter : [];
  return new Map(arr.filter((p) => p?.id).map((p) => [p.id, p]));
}

function mergeProduct(master = {}, override = {}, extras = {}) {
  return {
    ...master,
    ...override,
    ...extras,
    rolle: override?.reportRole || override?.rolle || master?.rolle || '',
    benchmark: override?.reportBenchmark || override?.benchmark || master?.benchmark || '',
    risikonivaa: override?.reportRisk || override?.risikonivaa || master?.risikonivaa || '',
    likviditet: override?.reportLiquidity || override?.likviditet || master?.likviditet || '',
    caseKort: override?.reportPitch || override?.caseKort || master?.caseKort || '',
    caseLang: override?.reportCaseLong || override?.caseLang || master?.caseLang || '',
    expectedReturn: override?.reportExpectedReturn ?? master?.expectedReturn ?? null,
    expectedYield: override?.reportExpectedYield ?? master?.expectedYield ?? null,
    reportTemplateName: override?.reportTemplateName || master?.slideTemplate || ''
  };
}
...
export function buildProposalModelV2({ formData = {}, monthlyDataset = null } = {}) {
  ...
  const productOverrideMap = buildOverrideMap(formData.pensumProdukter);

  const idsWithWeight = allocations.filter((item) => num(item.vekt) > 0).map((item) => item.id);
  const effectiveSelectedIds = (selectedIds.length > 0 ? selectedIds.filter((id) => idsWithWeight.includes(id)) : idsWithWeight);

  const selectedProducts = effectiveSelectedIds
    .map((id) => {
      const master = PENSUM_PRODUCT_MASTER_V2[id] || { id, navn: id, kortnavn: id, standardSlides: ['product-summary'] };
      const override = productOverrideMap.get(id) || {};
      return mergeProduct(master, override, {
        portfolioWeight: portfolioWeightMap.get(id) ?? 0,
        marketData: monthlyDataset?.products?.find((p) => p.productId === id) || null,
        exposureData: override?.eksponering || defaultProduktEksponering[id] || null,
        historikk: override?.historikk || null,
        slideRegistry: PRODUCT_SLIDE_REGISTRY_V2[id] || null
      });
    })
    .filter((p) => p.portfolioWeight > 0);
  ...
  return {
    meta: {
      reportDate: monthlyDataset?.reportDate || formData?.dato || null,
    ...
    portfolio: {
      summaryBullets,
      allocations: (formData.allokering || []).map((item) => ({ navn: item.navn, vekt: num(item.andel ?? item.vekt) })),
      selectedProducts: selectedProducts.map((p) => ({
        ...
        reportTemplateName: p.reportTemplateName,
