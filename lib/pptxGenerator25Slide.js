...
import {
  buildHistorikkVsReferanserChartSpec,
  buildMaxDrawdownChartSpec,
  buildAnnualHistoricalReturnsChartSpec,
  buildRegionExposureChartSpec,
  buildSectorExposureChartSpec,
} from './charting/builders/portfolioChartBuilders.js';
import { renderPptChartFromSpec } from './charting/renderers/ppt/renderFromSpec.js';
...
function buildProductHistorikkChart(produktHistorikk, productId, months) {
  ...
  sliced.forEach((pt, i) => {
    ...
    const normalized = (pt.verdi / baseVal) * 100;
...
export async function generateProposal25SlidePptx(payload) {
  ...
  const historikkSpec = buildHistorikkVsReferanserChartSpec(payload);
  if (historikkSpec) {
    renderPptChartFromSpec(pptx, slide, historikkSpec, { x: LAYOUT.marginL, y: 1.3, w: LAYOUT.contentW, h: 3.6 });
  }
  ...
  const drawdownSpec = buildMaxDrawdownChartSpec(payload);
  const ddData = drawdownSpec?.series?.[0]
    ? {
      labels: drawdownSpec.series[0].labels,
      drawdownValues: drawdownSpec.series[0].values,
      maxDrawdown: Math.min(...drawdownSpec.series[0].values),
    }
    : null;
  if (drawdownSpec) {
    renderPptChartFromSpec(pptx, slide, drawdownSpec, { x: LAYOUT.marginL, y: 1.3, w: 8.0, h: 3.0 });
  ...
  const annualSpec = buildAnnualHistoricalReturnsChartSpec(payload);
  if (annualSpec) {
    renderPptChartFromSpec(pptx, slide, annualSpec, { x: LAYOUT.marginL, y: 1.35, w: LAYOUT.contentW, h: 1.5 });
  }
  ...
  if (fullHistRows.length > 0) {
    ...
    slide.addTable(tData, {
      x: LAYOUT.marginL, y: 3.0, w: LAYOUT.contentW,
      border: { type: 'solid', pt: 0.5, color: C.line },
  ...
  const regionSpec = buildRegionExposureChartSpec(payload);
  const regioner = regionSpec ? regionSpec.categories.map((name, i) => ({ navn: name, vekt: regionSpec.values[i] })) : [];
  ...
  const sectorSpec = buildSectorExposureChartSpec(payload);
  const sektorer = sectorSpec ? sectorSpec.categories.map((name, i) => ({ navn: name, vekt: sectorSpec.values[i] })) : [];
  ...
  for (const prod of produkterMedVekt) {
    ...
    if (prodHistChart && prodHistChart.labels.length > 3) {
      ...
      renderPptChartFromSpec(pptx, slide, {
        type: 'line',
        title: prodName,
        series: [{
          name: prodName,
          labels: prodHistChart.labels,
          values: prodHistChart.values,
          color: safeColor(prod.color || C.navy),
        }],
        style: { yNumFmt: '#,##0', theme: { colors: { white: C.white, line: C.line, muted: C.muted, text: C.text } } },
      }, {
