...
function buildDrawdownData(payload) {
  ...
  fiveYearsDates.forEach((dato, di) => {
    ...
    if (totalWeight > 0) {
      portfolioSeries.push({ dato, value: (weightedReturn / totalWeight) * 100 });
    }
  });
...
export async function generateProposal25SlidePptx(payload) {
  ...
  mandatFacts.forEach(([label, val], i) => {
    ...
    slide.addShape(pptx.ShapeType.rect, { x: 0.7, y: iy, w: 0.06, h: 0.35, fill: { color: accentCol } });
    slide.addText(label, {
  ...
  ['kjerne', 'stabilisator', 'satellitt'].forEach(roleKey => {
    ...
    slide.addShape(pptx.ShapeType.roundRect, {
      x: 0.7, y: gy, w: 11.9, h: 0.38, rectRadius: 0.06, fill: { color: group.color },
    });
    ...
    items.forEach(p => {
      ...
      slide.addShape(pptx.ShapeType.ellipse, {
        x: 0.9, y: gy + 0.1, w: 0.22, h: 0.22, fill: { color: p.color },
      });
  ...
  if (histChartData && histChartData.labels.length > 3) {
    slide.addChart(pptx.ChartType.line, [{
      ...
      showTitle: false, lineDataSymbol: 'none', lineSmooth: true, lineSize: 2.5,
      chartColors: [C.navy],
  ...
  if (ddData && ddData.labels.length > 5) {
    slide.addChart(pptx.ChartType.area, [{
      ...
      showLegend: false, showTitle: false, lineDataSymbol: 'none', lineSmooth: true, lineSize: 1.5,
      chartColors: ['B91C1C'],
      catAxisLabelFontFace: FONT, catAxisLabelFontSize: 7, catAxisLabelColor: C.muted,
      valAxisLabelFontFace: FONT, valAxisLabelFontSize: 7, valAxisLabelColor: C.muted,
      valAxisNumFmt: '0.0"%"',
  ...
  if (riskItems.length > 0) {
    ...
    riskItems.forEach(([label, val, accentCol], i) => {
      ...
      slide.addShape(pptx.ShapeType.rect, { x: rx + 0.15, y: iy + 0.02, w: 0.05, h: 0.28, fill: { color: accentCol } });
  ...
  for (const prod of produkterMedVekt) {
    ...
    if (prodHistChart && prodHistChart.labels.length > 3) {
      ...
      slide.addChart(pptx.ChartType.line, [{
        ...
        showLegend: false, showTitle: false, lineDataSymbol: 'none', lineSmooth: true, lineSize: 2,
        chartColors: [prod.color || C.navy],
        catAxisLabelFontFace: FONT, catAxisLabelFontSize: 6.5, catAxisLabelColor: C.muted,
        valAxisLabelFontFace: FONT, valAxisLabelFontSize: 6.5, valAxisLabelColor: C.muted,
        valAxisNumFmt: '#,##0', plotArea: { fill: { color: C.white } },
      });
