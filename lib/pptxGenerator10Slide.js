...
export async function generateProposal10SlidePptx(payload) {
  ...
  mandatFacts.forEach(([label, val], i) => {
    ...
    slide.addShape(pptx.ShapeType.rect, { x: 0.7, y: iy, w: 0.06, h: 0.38, fill: { color: safeColor(accentCol) } });
    slide.addText(label, {
  ...
  ['kjerne', 'stabilisator', 'satellitt'].forEach(roleKey => {
    ...
    slide.addShape(pptx.ShapeType.roundRect, {
      ...
      fill: { color: safeColor(group.color) },
    });
  ...
  if (historikkSpec) {
    renderPptChartFromSpec(pptx, slide, historikkSpec, { x: LAYOUT.marginL, y: 1.3, w: 11.9, h: 3.3 });
  }
  ...
  const ddData = drawdownSpec?.series?.[0]
    ...
    : null;
  if (drawdownSpec) {
    renderPptChartFromSpec(pptx, slide, drawdownSpec, { x: LAYOUT.marginL, y: 1.35, w: 8.0, h: 3.2 });
  }
  ...
  if (riskItems.length > 0) {
    ...
    riskItems.forEach(([label, val, accentCol], i) => {
      ...
      slide.addShape(pptx.ShapeType.rect, {
        ...
        fill: { color: safeColor(accentCol) },
      });
      slide.addText(label, {
  ...
  // Region + sektor (hentet via felles chart-spec builders)
  const regionSpec = buildRegionExposureChartSpec(payload);
  if (regionSpec?.categories?.length) {
    slide.addText('Regioneksponering', {
      x: LAYOUT.marginL, y: 1.3, w: 5, h: 0.25,
      fontFace: FONT, ...TYPE.sectionTitle, color: C.navy,
    });
    const regioner = regionSpec.categories.map((name, i) => ({ name, value: regionSpec.values[i], color: C.accent }));
    addHBarChart(pptx, slide, regioner, LAYOUT.marginL, 1.65, 5.5, { labelW: 1.6 });
  }

  const sectorSpec = buildSectorExposureChartSpec(payload);
  if (sectorSpec?.categories?.length) {
    slide.addText('Sektoreksponering', {
      x: 6.8, y: 1.3, w: 5, h: 0.25,
      fontFace: FONT, ...TYPE.sectionTitle, color: C.navy,
    });
    const sektorer = sectorSpec.categories.map((name, i) => ({ name, value: sectorSpec.values[i], color: C.salmon }));
    addHBarChart(pptx, slide, sektorer, 6.8, 1.65, 5.8, { labelW: 2.0 });
  }
