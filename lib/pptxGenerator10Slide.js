...
function buildDrawdownData(payload) {
  ...
  // Build full daily portfolio series first (no sampling yet)
  const dailySeries = [];
  let baseValues = null;
  ...
  // Calculate drawdown from running peak on full daily data
  ...
  // Sample weekly (~every 5 trading days) for chart - single pass
  const drawdownValues = [];
  const labels = [];
  let maxDD = 0;

  fullDrawdown.forEach((pt, i) => {
    // Always include first, last, and weekly points
...
export async function generateProposal10SlidePptx(payload) {
  ...
  if (histChartData && histChartData.labels.length > 3) {
    ...
    slide.addChart(pptx.ChartType.line, [{
      ...
      catAxisLineColor: safeColor(C.line),
