...
/**
 * Beregn statistikk fra månedlig historikkdata for en gitt periode.
...
export function beregnProduktStatistikk(historikkData, startDato, riskFreeRate = 3) {
  ...
  const filtrert = byggMaanedssluttSerie(historikkData.data)
    .filter(d => {
  ...
  const fullSerie = byggMaanedssluttSerie(historikkData.data)
    .filter(d => erGyldigTall(d.verdi))
    .sort((a, b) => parseHistorikkDato(a.dato) - parseHistorikkDato(b.dato));
  let avkSidenOppstart = null;
  if (fullSerie.length >= 2) {
    const nFull = fullSerie.length - 1;
    avkSidenOppstart = ((fullSerie[fullSerie.length - 1].verdi / fullSerie[0].verdi) ** (perioderPerAar / nFull) - 1) * 100;
  }
...
/**
 * Beregn korrelasjonsmatrise mellom produkter basert på månedlige avkastninger.
...
export function beregnKorrelasjonsmatrise(historikkMap, startDato) {
  ...
  // Bygg månedlige avkastningsserier per produkt
  const avkastningSerier = {};
  const alleDatoer = new Set();

  ids.forEach(id => {
    const serie = byggMaanedssluttSerie(historikkMap[id].data)
      .filter(d => {
        const parsed = parseHistorikkDato(d.dato);
        return parsed && (!startDato || parsed >= startDato) && erGyldigTall(d.verdi);
      })
