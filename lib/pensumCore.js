...
export const TEMPLATE_FILENAME_CANDIDATES = [
  'Mal - Forslag til investeringsportefolje 2026.pptx',
  'Mal - Forslag til investeringsportefølje 2026.pptx'
];
export const DEFAULT_TEMPLATE_FILENAME = TEMPLATE_FILENAME_CANDIDATES[0];
export const erPptTemplateFilnavn = (filnavn = '') => /\.(ppt|pptx)$/i.test(String(filnavn || '').trim());
export const normalizeTemplateFilename = (filnavn = '') => String(filnavn || '')
  .trim()
  .toLowerCase()
  .normalize('NFD')
  .replace(/[̀-ͯ]/g, '');
export const erStandardTemplateFilnavn = (filnavn = '') => {
  const normalized = normalizeTemplateFilename(filnavn);
  return TEMPLATE_FILENAME_CANDIDATES.some((candidate) => normalizeTemplateFilename(candidate) === normalized);
};
...
export const DEFAULT_LIKVID = 8000000;  // 3 mill aksjefond + 1 mill aksjer + 2 mill renter + 2 mill kontanter
